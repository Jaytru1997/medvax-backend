...
const flw = new Flutterwave("your-flutterwave-api-key");
...
exports.createPaymentLink = async (req, res) => {
    try {
        ...
        if (response.status === "success") {
            return res.json({ message: "Payment link created", link: response.data.link });
        } else {
            return res.status(400).json({ message: "Error creating payment link", error: response });
        }
    } catch (error) {
        res.status(500).json({ message: "Error processing payment", error: error.response?.data });
    }
};
